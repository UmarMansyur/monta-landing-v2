    ...
    <nav class="fixed inset-x-0 top-0 z-50 bg-white/60 sm:bg-transparent sm:backdrop-blur-3xl" aria-label="Main navigation">
      <div class="lg:container sm:px-3 flex flex-wrap items-center justify-between mx-auto py-3 p-2 sm:py-3">
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto bg-white md:order-1 sm:bg-transparent" id="navbar-cta">
          <ul class="flex flex-col font-medium p-4 md:p-0  md:space-x-4 lg:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 ">
            <li>
              <RouterLink to="/" class="md:bg-transparent sm:text-sm md:text-sm lg:text-base ps-3"aria-current="page" :class="{ 'active': $route.path === '/', 'inactive': $route.path !== '/' }">
                Beranda
              </RouterLink>
            </li>
            <li class="relative hover:text-blue-600">
              <button type="button" @click="clickMenu" @mouseenter="mouseHover" @mouseleave="hiddenCollapse" id="dropdownNavbarLink" data-dropdown-toggle="dropdownNavbar" class="flex items-center justify-between w-full py-2 px-3 rounded hover:text-blue md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 md:w-auto text-sm sm:text-sm md:text-sm lg:text-base hover:textblue">
                <span :class="{ 'active': $route.path.includes('pengajuan') }">
                  Pengajuan KP/TA
                </span>
                <svg class="w-2.5 h-2.5 ms-2.5" :class="$route.path.includes('pengajuan') ? 'text-blue-600': ''" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                  viewBox="0 0 10 6">
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="m1 1 4 4 4-4" />
                </svg>
            ...
            <li>
              <RouterLink to="/penelitian-dosen" class="block py-2 px-3 md:p-0 rounded md:hover:text-blue-700 text-sm sm:text-sm md:text-sm lg:text-base">
                Penelitian Dosen
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/panduan" class="block py-2 px-3 md:p-0 rounded md:hover:text-blue-700 text-sm sm:text-sm md:text-sm lg:text-base">
                Panduan
              </RouterLink>
            </li>
          </ul>
